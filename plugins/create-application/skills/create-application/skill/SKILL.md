...
mcp__perplexity__perplexity_search(query="[App Name] official website URL - find from authoritative sources")
mcp__perplexity__perplexity_search(query="[App Name] hosting type: Is it SaaS, PaaS, or IaaS? Is it for developers, end-users, or IT teams? What cloud provider?")
mcp__perplexity__perplexity_search(query="Does [App Name] support SSO (Single Sign-On)? Check for SAML, OAuth, Azure AD, Okta integration")
mcp__perplexity__perplexity_search(query="[App Name] pricing model and costs - free tier, subscription, usage-based, or enterprise pricing?")
mcp__perplexity__perplexity_search(query="What product category is [App Name]? Is it cloud platform, developer tool, CRM, project management, etc.?")
mcp__perplexity__perplexity_search(query="[App Name] aliases, former names, abbreviations, or alternate names")
mcp__perplexity__perplexity_search(query="Is [App Name] a web application or mobile app? Browser-based or native?")
mcp__perplexity__perplexity_search(query="[App Name] description - what does it do? Core capabilities? Factual description without marketing")
```
